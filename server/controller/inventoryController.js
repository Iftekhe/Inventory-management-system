...
exports.approveInventoryById = async (req, res) => {
  try {
    ...
    if (approvedInventory) {
      ...
      approvedInventory.status = 'approved';
      await approvedInventory.save();
    }
...
//     // Find and update the inventory record for the given product and location, or create a new one if it doesn't exist
//     let inventoryRecord = await Inventory.findOneAndUpdate(
//       { productId, locationId, status: 'approved' },
...
exports.getAllInventory = async (req, res) => {
  try {
    const inventoryItems = await Inventory.find({ status: 'approved' })
      .populate('productId')
      .populate('locationId')
      // .populate({
      //   path: 'branchId',
